...
async function fetchKpiData(kpiView, requestedKpiList, leadSource, gte, lte, department, teamMemberStrings) {
  ...
  // console.log("lte ", lte);
   ...
   console.log("teamMember ", teamMemberStrings);
  // convert the teamMember array of strings to an array of integers
  ...
  try {
    ...
    const generateFilters = (fieldName, dateFieldName, extraFilters) => {
      ...
      if (leadSource && leadSource.length > 0 && fieldName === "Lead Source") {
        filters.push({
          "type": "app",
          "fieldName": fieldName,
          "values": leadSource,
