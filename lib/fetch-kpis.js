...
async function fetchKpiData(view, kpiList, leadSource, gte, lte) {
  ...
  try {
    ...
    function getKpiValue(kpiName, calculatedKPIs, endpointData, dataKey) {
    ...
    const kpiObjects = kpiDefinitionsArray.filter((kpiDefinition) => kpiList.includes(kpiDefinition.name))
      .map((kpiDefinition) => {
        ...
        const data1 = createDataString(dataLabels[0], getKpiValue(kpiDefinition.name, calculatedKPIs, endpointData, kpiDefinition.dataKeys[0]));
        const data2 = createDataString(dataLabels[1], getKpiValue(kpiDefinition.name, calculatedKPIs, endpointData, kpiDefinition.dataKeys[1]));
